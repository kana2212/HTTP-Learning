...
https:// はスキーム名、weatherはホスト名、yahooはドメイン名、coはセカンドレベルドメイン、jpはトップレベルドメイン、  
/weather/はディレクトリ、
...
ドメインの後の?より前にくる特定のリソースを識別する為に必要な情報。パス(URL)の一部分をパラメータ化して関数部分で利用する事を  
可能にする仕組み。
...
使用例・・・API経由でデータを取得、画像データの取得  
  ...
  ---PATCHの意味  
  データが既に存在しているものに対して更新をかける処理の事。部分的な更新。  
  元のリソース以外のものに変更を適用する、又は変更を複数回適用するとリソースが無効になる可能性がある為、冪等性はないと言える 
  ...
  主なフィールドはホスト、参照元URL、Cookie、Authorizationなどがある
  
  ### ・リクエストボディとは何か  
  HTTPリクエストを構成する部品で、主に補足情報が書かれている。POSTリクエストの場合は受け渡すパラメータの内容が書かれる  
